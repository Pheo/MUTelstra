...
def mTrain(string):
    ...
    for word in words:
        if word.strip(',').lower() in tLines:
            affected_d['lines'].append(word.strip(','))
    ...
    # DICTIONARY_TYPE
    affected_d['dates'] = dateHandler(string[n:])

    # Call store_data 
    store_data('MTRAIN', affected_d)
...
def mTram(string):
    ...
    affected_d = {}
    affected_d['routeDisruption'] = routeDisruption
    affected_d['time'] = dateHandler(string[index:])

    # Call store_data 
    store_data('MTRAM', affected_d)
